...
CREMeshWriter::CREMeshWriter(scene::ISceneManager* smgr, io::IFileSystem* fs)
...
EMESH_WRITER_TYPE CREMeshWriter::getType() const
{
    return EMWT_OBJ;
}


void CREMeshWriter::createWeightsTable(ISkinnedMesh* mesh)
{
    _table.clear();
...
int CREMeshWriter::getJointsSize(ISkinnedMesh* mesh)
{
...
int CREMeshWriter::getMaterialsSize(IMesh* mesh)
{
    int size = 0;
    for (u32 i = 0; i < mesh->getMeshBufferCount(); ++i)
    {
...
//! writes a mesh
bool CREMeshWriter::writeAnimatedMesh(io::IWriteFile* file, scene::IMesh* mesh, bool skinned, s32 flags)
{
	if (!file)
		return false;
...
void CREMeshWriter::writeCollisionMesh(io::IWriteFile* file)
{
...
void CREMeshWriter::writeLOD(io::IWriteFile* file, core::stringc lodName, IMesh* lodMesh, bool skinned)
{
...
void CREMeshWriter::setCollisionMesh(IMesh *mesh)
{
    CollisionMesh = mesh;
}

void CREMeshWriter::clearLODS()
{
    MeshLOD1 = 0;
    MeshLOD2 = 0;
    CollisionMesh = 0;
